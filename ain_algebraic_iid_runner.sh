...
dataset=$1
experiment=$2
seeds=${3:-"1,2,3"} # comma- or space-separated list, e.g. 1,2,3 o "4 5"
model="split_resnet_algebraic_non_iid"

if [ -z "$dataset" ] || [ -z "$experiment" ]; then
    echo "Usage: $0 <dataset> <experiment> [seeds]"
    echo "  seeds: comma-separated list of seeds to run (default: 1,2,3)"
    echo "         e.g. $0 dsprites comp_ablation_iid 4,5"
    exit 1
fi
...
data_cfg="configs/datasets/${dataset}_iid.yml"

...
def select_best_test(train_data):
    bests = defaultdict(tuple)
    all = []
    for v in train_data.values():
        curr = v["test_acc"]
        if bests["test"] == tuple() or curr >= bests["test"][0]:
            bests["test"] = (curr, v)
        all.append([curr])
...
def parse_training_metrics(path):
    ...
    best_epoch_results, curves = select_best_test(epoch_data)
...
def select_best_test_id(train_data):
    bests = (0, 0, 0, 0)
    for v in train_data.values():
        if v["test_acc"] >= bests[3]:
...
def parse_id(path):
    ...
    best_epoch_result = select_best_test_id(epoch_data)
    return {
    ...
    for c_path in c_list:
        ...
        for model_path in models:
            ...
            for comb in combinations:
                ...
                for r in runs:
                    try:
                        run_id = os.path.basename(r).split("/")[-1]
                        if PARSE_LOG:

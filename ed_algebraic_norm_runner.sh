...
dataset=$1
experiment=$2
model=${3:-"ed_algebraic_relnorm"}   # ed_algebraic_relnorm (arreglo) | ed_algebraic_non_iid (control)
seeds=${4:-"1,2,3"}                  # comma- or space-separated list

if [ -z "$dataset" ] || [ -z "$experiment" ]; then
    echo "Usage: $0 <dataset> <experiment> [model] [seeds]"
    echo "  model: ed_algebraic_relnorm (default) o ed_algebraic_non_iid"
    echo "  seeds: comma-separated list (default: 1,2,3)"
    exit 1
fi

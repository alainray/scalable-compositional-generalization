...
# Baseline de AIN SIN mixer, variando el numero de bloques exclusivos.
# Replica la configuracion con la que ya se corrio mpi3d (split_1..split_4):
# data cfg plano (sin non_iid) y arch split_resnet.

dataset=$1
experiment=$2
layer=$3                      # 1..4 bloques exclusivos
seeds=${4:-"1,2,3,4,5"}
model="split_${layer}"

if [ -z "$dataset" ] || [ -z "$experiment" ] || [ -z "$layer" ]; then
    echo "Usage: $0 <dataset> <experiment> <layer> [seeds]"
    exit 1
fi
...
for c in "${C[@]}"; do
    for seed in "${SEEDS[@]}"; do
        ...
        --seed=$seed data.training.num_workers=4 data.testing.num_workers=4 logger.name=wandb \

...
def get_split_resnet_mixer(**cfg):
	...
	head=get_net(in_dim=emb_dim,out_dim=out_dim,arch='fc_vec',l_hidden=[],activation=[],out_activation=out_activation);return get_net(in_channels=in_chan,out_dim=emb_dim,preprocessing=preprocessing,head=head,attributes=attribute_names,attribute_sizes=attribute_sizes,objective=objective,loss_fn=loss,metric_fns=metrics,mixer_num_layers=mixer_cfg.get('num_layers',2),mixer_num_heads=mixer_cfg.get('num_heads',4),mixer_dropout=mixer_cfg.get('dropout',0.0),mixer_loss_weight=mixer_cfg.get('loss_weight',1.0),mixer_detach_target=mixer_cfg.get('detach_target',False),use_mixer_classifier=mixer_cfg.get('use_classifier',False),use_all_mixer_cases=mixer_cfg.get('use_all_cases',False),**model_cfg)

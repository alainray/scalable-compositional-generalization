...
def get_resnet(**cfg):
	...
	head=get_net(in_dim=emb_dim,out_dim=out_dim,arch='fc_vec',l_hidden=[],activation=[],out_activation=out_activation,bias=model_cfg.get('head_bias',True));return get_net(in_channels=in_chan,out_dim=emb_dim,preprocessing=preprocessing,head=head,attributes=attribute_names,attribute_sizes=attribute_sizes,objective=objective,loss_fn=loss,metric_fns=metrics,**model_cfg)
def get_resnet_mixer(**cfg):
	...
	classifier=get_net(in_dim=emb_dim,out_dim=out_dim,arch='fc_vec',l_hidden=[],activation=[],out_activation=out_activation,bias=model_cfg.get('head_bias',True))
...
def get_net(**kwargs):
	...
	elif arch in['split_resnet','split_resnet_mixer']:comp_classes={'split_resnet':SplitResNet18,'split_resnet_mixer':SplitResNet18Mixer};args=kwargs|{'activation':get_activation(kwargs['activation'])};return comp_classes[arch](**args)
...
def get_split_resnet(**cfg):
	...
	head=get_net(in_dim=emb_dim,out_dim=out_dim,arch='fc_vec',l_hidden=[],activation=[],out_activation=out_activation,bias=model_cfg.get('head_bias',True));return get_net(in_channels=in_chan,out_dim=emb_dim,preprocessing=preprocessing,head=head,attributes=attribute_names,attribute_sizes=attribute_sizes,objective=objective,loss_fn=loss,metric_fns=metrics,**model_cfg)
def get_split_resnet_mixer(**cfg):
	model_cfg=cfg['model'];in_chan=model_cfg['feature_extraction']['in_channels'];emb_dim=model_cfg['emb_dim'];out_activation=model_cfg['out_activation'];loss=get_loss(cfg['training']['loss']);objective=cfg['training']['objective'];metrics=get_metrics(cfg['training']['metrics']);attribute_names,attribute_sizes,_=_get_attribute_info(cfg);preprocessing=_get_preprocessing(model_cfg.pop('preprocessing'));mixer_cfg=model_cfg.pop('mixer',{})
	if objective=='classification':out_dim=sum(attribute_sizes)
	elif objective=='regression':out_dim=len(attribute_names)
	else:raise NotImplementedError(f"Objective {objective} is not supported.")
	head=get_net(in_dim=emb_dim,out_dim=out_dim,arch='fc_vec',l_hidden=[],activation=[],out_activation=out_activation,bias=model_cfg.get('head_bias',True));return get_net(in_channels=in_chan,out_dim=emb_dim,preprocessing=preprocessing,head=head,attributes=attribute_names,attribute_sizes=attribute_sizes,objective=objective,loss_fn=loss,metric_fns=metrics,mixer_num_layers=mixer_cfg.get('num_layers',2),mixer_num_heads=mixer_cfg.get('num_heads',4),mixer_dropout=mixer_cfg.get('dropout',0.0),mixer_rep_dim=mixer_cfg.get('rep_dim',None),mixer_rep_piece_dim=mixer_cfg.get('rep_piece_dim',None),mixer_loss_weight=mixer_cfg.get('loss_weight',1.0),mixer_detach_target=mixer_cfg.get('detach_target',False),use_mixer_classifier=mixer_cfg.get('use_classifier',False),use_all_mixer_cases=mixer_cfg.get('use_all_cases',False),mixer_mode=mixer_cfg.get('mode','transformer'),algebraic_use_all_terms=mixer_cfg.get('algebraic_use_all_terms',True),**model_cfg)
